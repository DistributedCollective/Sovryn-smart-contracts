...
describe("LiquidityMiningMigration", () => {
    ...
    beforeEach(async () => {
        ...
        await migrator.initialize(
            ...
            liquidityMiningV2.address
        );

        await liquidityMiningV2.initialize(wrapper.address, migrator.address);
        ...
        await liquidityMiningV2.addRewardToken(
            SOVToken.address,
            rewardTokensPerBlock,
            startDelayBlocks,
    ...
    describe("initializeLiquidityMining", () => {
        it("should check all user deposits", async () => {
            for (let i = 0; i < accountDeposits.length; i++) {
                for (let j = 0; j < accountDeposits[i].deposit.length; j++) {
                    ...
                    let { amount } = await liquidityMining.userInfoMap(
                        poolId,
        ...
        it("should fail if liquidity mining V1 address is invalid in migrator contract", async () => {
            migrator = await Migrator.new();
            await expectRevert(
                migrator.initialize(SOVToken.address, ZERO_ADDRESS, liquidityMiningV2.address),
        ...
        it("should fail if liquidity mining V2 address is invalid in migrator contract", async () => {
            migrator = await Migrator.new();
            await expectRevert(
                migrator.initialize(SOVToken.address, liquidityMining.address, ZERO_ADDRESS),
                "invalid contract address"
            );
        });
    ...
    describe("migratePools", () => {
        ...
        it("should only allow to migrate pools by migrator contract", async () => {
            ...
            await liquidityMiningV2.initialize(wrapper.address, SOVToken.address);
            await liquidityMining.addAdmin(migrator.address);
            await liquidityMining.startMigrationGracePeriod();
        ...
        it("should add pools from liquidityMininigV1", async () => {
            ...
            for (let i = 0; i < tokens.length; i++) {
                ...
                let { startBlock: startBlockV2, totalUsersBalance: totalUsersBalanceV2 } =
                    await liquidityMiningV2.rewardTokensMap(SOVToken.address);
        ...
        it("should be able to migrate users in differents tx", async () => {
            ...
            for (let i = 0; i < tokens.length; i++) {
                ...
                for (let j = 0; j < accountDeposits.length; j++) {
                    let userInfo = await liquidityMining.getUserInfo(
                        tokens[i].address,
                        accountDeposits[j].account
                    );
            ...
            for (let i = 0; i < tokens.length; i++) {
                for (let j = 0; j < accountDeposits.length; j++) {
                    let userInfoV2 = await liquidityMiningV2.getUserInfo(
                        tokens[i].address,
                        accountDeposits[j].account
                    );

                    expect(userInfoV2.amount).bignumber.equal(userInfoV1[i][j].amount);
                    expect(userInfoV2.rewards[0].rewardDebt).bignumber.equal(
                        userInfoV1[i][j].rewardDebt
                    );
                    expect(userInfoV2.rewards[0].accumulatedReward).bignumber.equal(
                        userInfoV1[i][j].accumulatedReward
                    );
                }
            }
        });
    ...
    describe("withdraws", () => {
        it("should withdraw all before migration and revert trying to withdraw after", async () => {
            await liquidityMining.withdraw(
                ...
                accountDeposits[0].deposit[0].amount,
                ZERO_ADDRESS,
                {
        ...
        it("should withdraw half before migration and withdraw the other half after", async () => {
            ...
            expect(tokenBalanceAfter.sub(tokenBalanceBefore)).bignumber.equal(
                accountDeposits[0].deposit[0].amount
            );
        });
        it("should withdraw all before migration, migrate, deposit and withdraw all again", async () => {
            ...
            await token1.approve(liquidityMining.address, accountDeposits[0].deposit[0].amount, {
                from: accountDeposits[0].account,
            });
            ...
            await token1.approve(liquidityMiningV2.address, accountDeposits[0].deposit[0].amount, {
                from: accountDeposits[0].account,
            });
            await liquidityMiningV2.deposit(
                token1.address,
                accountDeposits[0].deposit[0].amount,
                ZERO_ADDRESS,
                {
        ...
        it("should get rewards in liquidity mining V2 after migration", async () => {
            ...
            tx = await liquidityMiningV2.withdraw(
                token1.address,
                accountDeposits[0].deposit[0].amount.div(new BN(2)),
                ZERO_ADDRESS,
                {
        ...
        it("should migrate rewards", async () => {
            ...
            await liquidityMining.claimReward(token1.address, ZERO_ADDRESS, {
                from: accountDeposits[0].account,
            });
            let { rewardDebt: rewardDebtBefore } = await liquidityMining.userInfoMap(
                0,
                accountDeposits[0].account
            );
        ...
        it("should be able to claim rewards after migration", async () => {
            ...
            tx = await liquidityMining.claimReward(token1.address, ZERO_ADDRESS, {
                from: accountDeposits[0].account,
            });
            let blockStart = tx.receipt.blockNumber;
            let { rewardDebt: rewardDebtBefore } = await liquidityMining.userInfoMap(
            ...
            tx = await liquidityMiningV2.claimRewards(token1.address, ZERO_ADDRESS, {
                from: accountDeposits[0].account,
            });
            let blockEnd = tx.receipt.blockNumber;
            let passedBlocks = new BN(blockEnd - blockStart);
    ...
    describe("Test lending with liquidity mining V2", async () => {
        ...
        it("Should only allow to burn tokens if migration finished", async () => {
            ...
            let userInfo = await liquidityMiningV2.getUserInfo(loanToken.address, lender);
            await expectRevert(
                loanToken.burn(lender, userInfo.amount, true),
    ...
    describe("Test WRBTC lending with liquidity mining", () => {
        ...
        it("Should only allow to burn tokens if migration finished", async () => {
            ...
            await expectRevert(
                loanTokenWRBTC.burnToBTC(lender, userInfo.amount, true),
                "Migration is not over yet"
            );
        });
    ...
    async function initializeLiquidityMiningDeposits() {
        accountDeposits.forEach((account) => {
            account.deposit.forEach(async (deposit) => {
                await liquidityMining.deposit(deposit.token, deposit.amount, ZERO_ADDRESS, {
