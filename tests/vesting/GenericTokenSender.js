...
contract("GenericTokenSender", (accounts) => {
    ...
    describe("transferTokens", () => {
        ...
        it("only owner or admin should be able to transfer", async () => {
            await expectRevert(
                tokenSender.transferTokens(SOV.address, account1, 1000, { from: account1 }),
                "unauthorized"
            );
        });
        ...
        it("fails if the 0 address is passed as receiver address", async () => {
            await expectRevert(
                tokenSender.transferTokens(SOV.address, ZERO_ADDRESS, 1000),
        ...
        it("fails if the 0 is passed as an amount", async () => {
            await expectRevert(
                tokenSender.transferTokens(SOV.address, account1, 0),
                "amount invalid"
            );
        });
    });
    ...
    describe("transferTokensUsingList", () => {
        it("should be able to transfer SOV", async () => {
            ...
            await tokenSender.addAdmin(account1);
            let tx = await tokenSender.transferTokensUsingList(SOV.address, [account1], [amount], {
                from: account1,
            });
        ...
        it("should be able to transfer SOV to N users", async () => {
            ...
            await tokenSender.addAdmin(account1);
            let tx = await tokenSender.transferTokensUsingList(
                SOV.address,
                [account1, account2],
                [amount, amount],
                { from: account1 }
        ...
        it("only owner or admin should be able to transfer", async () => {
            await expectRevert(
                tokenSender.transferTokensUsingList(SOV.address, [account1], [1000], {
                    from: account1,
                }),
                "unauthorized"
            );
        ...
        it("fails if the 0 address is passed as receiver address", async () => {
            await expectRevert(
                tokenSender.transferTokensUsingList(SOV.address, [ZERO_ADDRESS], [1000]),
                "receiver address invalid"
            );
        });
        ...
        it("fails if the 0 is passed as an amount", async () => {
            await expectRevert(
                tokenSender.transferTokensUsingList(SOV.address, [account1], [0]),
                "amount invalid"
            );
        });
    });
