 ...
 * `colfee/docs/IMPLEMENTATION_DESIGN.md` §rollover, rollover payouts are
 * keeper/maintenance compensation and must not be ColFee-charged.
 *
 ...
 * any branch — so ColFee short-circuits: no `ExitFeeApplied`, no
 * `ExitFeeSkipped`. The borrower-self-rollover case covers the keeper-reward
 * branch with the borrower as caller.
 *
 * Run:
 *   npx hardhat test tests/colfee/Rollover.notouch.test.js
 */
...
contract("ColFee — Rollover no-touch coverage (Phase 3 / regression)", (accounts) => {
    ...
    async function deploymentAndInitFixture() {
        ...
        // ColFee controller wired and FULLY ACTIVE — this proves the
    ...
    // Value-level counterpart to the "no ColFee event" assertions: the fee
    ...
    // all assert "no ColFee event" would pass identically if the fee system
    ...
    describe("normal rollover (rolloverReward != 0 and <= collateral; loan stays open)", () => {
        it("ColFee does NOT fire — no _closeWithSwap at all, just interest renewal + keeper reward", async () => {
    ...
    describe("rollover tiny-position branch (Rollover.sol:244, allowDonationOnFailure = true)", () => {
        it("ColFee does NOT fire — exempt by CloseOrigin.Rollover (independent of allowDonationOnFailure)", async () => {
    ...
    describe("rollover keeper-reward branch (Rollover.sol:224, rolloverReward > collateral)", () => {
        it("ColFee does NOT fire — no ExitFeeApplied, no ExitFeeSkipped — and the keeper still receives the residual", async () => {
            const { borrower, loan_id } = await openMarginTradeAndExpire("tiny");
            ...
            // CloseOrigin.Rollover → `_exitFeeChargeable` returns false → ColFee
            ...
            // The borrower's loan is gone, but no ColFee was charged on the
            // keeper's payout.
            expect(borrower).to.not.equal(rolloverKeeper);
        });
    });
    ...
    describe("rollover keeper-reward branch (Rollover.sol:224) — BORROWER self-initiated", () => {
        it("ColFee does NOT fire even when msg.sender == borrower — exempt by CloseOrigin.Rollover", async () => {
            const { borrower, loan_id } = await openMarginTradeAndExpire("tiny");
