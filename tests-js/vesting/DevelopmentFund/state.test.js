 ...
 */
async function checkStatus(contractInstance, checkArray, lockedTokenOwner, unlockedTokenOwner, newLockedTokenOwner, lastReleaseTime, remainingTokens, releaseDuration, releaseTokenAmount){
    if(checkArray[0] == 1){
    ...
    if(checkArray[5] == 1){
        let cValue = [];
        await contractInstance.getReleaseDuration().then(
            data => {
                cValue = data;
            }
        );
        ...
        assert(cValue.every((value, index) => value === releaseDuration[index]), "The release duration does not match.");
    }
    if(checkArray[6] == 1){
        let cValue = [];
        await contractInstance.getReleaseTokenAmount().then(
            data => {
                cValue = data;
            }
        );
        ...
        assert(cValue.every((value, index) => value === releaseTokenAmount[index]), "The release token amount does not match.");
    }
}
...
contract("DevelopmentFund (State)", (accounts) => {
	...
	it("Successfully created an instance without any error.", async () => {
        ...
        await checkStatus(developmentFund, [1,1,0,1,0,0,0], governance, multisig, zero, currentTime, zero, 0, releaseDuration[0], releaseTokenAmount[0]);
    });
    ...
    it("The contract should be approved to make token transfer for deposit.", async () => {
        let value = randomValue();
        await expectRevert(
            developmentFund.depositTokens(value, {from: userOne}),
            "Not enough token approved for transfer.",
        );
    });
    
    it("Zero Tokens could not be deposited.", async () => {
        await expectRevert(
            developmentFund.depositTokens(0, {from: userOne}),
            "Amount needs to be bigger than zero.",
        );
    });
    ...
    it("Updating the release schedule should update the lastReleaseTime, releaseDuration and releaseTokenAmount.", async () => {
        ...
        await checkStatus(developmentFund, [0,0,0,1,1,1,1], zero, zero, zero, newReleaseTime, totalReleaseTokenAmount, releaseDuration, releaseTokenAmount);
    });
	...
	it("After withdrawing all tokens after a particular schedule, the release time should be updated based on duration.", async () => {
        ...
        await checkStatus(developmentFund, [0,0,0,1,0,0,0], zero, zero, zero, lastReleaseTime.toNumber() + releaseDuration[releaseDuration.length - 1], zero, releaseDuration, releaseTokenAmount);
    });
    ...
    it("Zero Tokens could not be withdrawed from release schedule.", async () => {
        await expectRevert(
            developmentFund.withdrawTokensByUnlockedTokenOwner(zero, {from: multisig}),
