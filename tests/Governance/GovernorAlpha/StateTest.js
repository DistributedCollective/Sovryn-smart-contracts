...
const governorAlphaPath = path.join(
    __dirname,
    "../../..",
    "contracts",
...
const states = Object.entries(statesInverted).reduce(
...
contract("GovernorAlpha#state/1", (accounts) => {
    ...
    async function deploymentAndInitFixture(_wallets, _provider) {
        ...
        await staking.stake(TWO_PERCENTAGE_VOTES, stakingDate, accounts[3], accounts[3], {
            from: accounts[3],
        });
    ...
    it("Pending", async () => {
        expect((await gov.state.call(trivialProposal.id)).toString()).to.be.equal(
            states["Pending"].toString()
        );
    });
    ...
    it("Defeated by minPercentageVotes", async () => {
        ...
        expect(proposal.forVotes).to.be.bignumber.lessThan(
            totalVotes.mul(new BN(MAJORITY)).div(new BN(100))
        );
    ...
    it("Shouldn't be canceled", async () => {
        ...
        // cancel proposal
        await expectRevert(
            gov.cancel(proposalId, { from: acct }),
            "GovernorAlpha::cancel: sender isn't a guardian"
        );
    });
