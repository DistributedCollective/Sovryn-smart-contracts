...
describe("ERC20TransferLogic", () => {
    ...
    describe("initialize", () => {
        it("fails if not an owner or admin", async () => {
            transferLogic = await ERC20TransferLogic.new();
            await expectRevert(
                transferLogic.initialize(token1.address, { from: account1 }),
                "unauthorized"
            );
    ...
    describe("setTokenAddress", () => {
        it("fails if not an owner or admin", async () => {
            await expectRevert(
                transferLogic.setTokenAddress(token1.address, { from: account1 }),
                "unauthorized"
            );
        ...
        it("fails if invalid address", async () => {
            await expectRevert(
                transferLogic.setTokenAddress(ZERO_ADDRESS),
    ...
    describe("transferReward", async () => {
        ...
        it("fails if invalid address to transfer", async () => {
            ...
            await token1.approve(transferLogic.address, account1InitialBalance, {
                from: account1,
            });
            await expectRevert(
        ...
        it("should account1 transfer reward to account2", async () => {
            ...
            await transferLogic.transferReward(account2, amountToTransfer, false, {
                from: account1,
            });
