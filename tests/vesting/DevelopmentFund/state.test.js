...
async function checkStatus(
    ...
    if (checkArray[4] == 1) {
        ...
        assert(
            cValue.length == releaseDuration.length,
            "The release duration length does not match."
        );
        assert(
    ...
    if (checkArray[5] == 1) {
        ...
        cValue = convertBNArrayToNumArray(cValue);
        assert(
            cValue.length == releaseTokenAmount.length,
            "The release token length does not match."
        );
        assert(
...
contract("DevelopmentFund (State)", (accounts) => {
    ...
    it("Updating the release schedule should update the lastReleaseTime, releaseDuration and releaseTokenAmount.", async () => {
        ...
        assert.equal(
            ...
            "Token Balance in contract does not match the correct amount."
        );
    });
    ...
    it("While updating release schedule, deficient tokens should be sent to contract.", async () => {
        ...
        assert.strictEqual(
            afterTokenBalance.toNumber(),
            beforeTokenBalance.toNumber() - deficient,
            "Extra tokens not sent to contract."
        );
        await checkStatus(
            developmentFund,
            [0, 0, 0, 1, 1, 1, 0],
    ...
    it("Unequal array for duration and tokens should not be accepted for token release schedule.", async () => {
        ...
        await expectRevert(
            developmentFund.changeTokenReleaseSchedule(
                newReleaseTime,
                releaseDuration,
                releaseTokenAmount,
                { from: governance }
    ...
    it("After withdrawing all tokens after 2 particular schedule, the release array size should decrease.", async () => {
        ...
        // Increasing the time to pass atleast one duration.
        await time.increase(
            releaseDuration[releaseDuration.length - 1] +
                releaseDuration[releaseDuration.length - 2] +
                1
        );

        await developmentFund.withdrawTokensByUnlockedTokenOwner(
            releaseTokenAmount[releaseTokenAmount.length - 1] +
                releaseTokenAmount[releaseTokenAmount.length - 2],
            { from: multisig }
        );
    ...
    it("After withdrawing part of tokens after a particular schedule, the release token amount should decrease.", async () => {
        ...
        await developmentFund.withdrawTokensByUnlockedTokenOwner(withdrawAmount, {
            from: multisig,
        });
        releaseTokenAmount[releaseTokenAmount.length - 1] =
            releaseTokenAmount[releaseTokenAmount.length - 1] - withdrawAmount;
        await checkStatus(
            developmentFund,
            [0, 0, 0, 0, 1, 1, 0],
            zero,
