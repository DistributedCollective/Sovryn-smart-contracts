/**
 * Shared setup for the ColFee Phase-1 activation SIP fork tests.
 *
 * The ExitFeeController/ExitFeeVault (colfee repo, 0.8.20) and the hooked Zero
 * BorrowerOperations (zero-contracts-colfee, 0.6.11) cannot be compiled by this
 ...
 * "BorrowerOperationsColFee") — which is exactly how the `<TBD>` production
...
const borrowerOperationsFixture = require("./fixtures/BorrowerOperationsColFee.json");
const collSurplusPoolFixture = require("./fixtures/CollSurplusPoolColFee.json");
...
const deployColFeeStack = async (deployerSigner, rateBps) => {
    ...
    if (await controller.exitFeeEnabled()) {
        throw new Error("ColFee stack must deploy disabled");
    }
 ...
 *  zero-contracts-colfee @ private/colfee) and save the
 *  "BorrowerOperationsColFee" record the sipArgs Part 1 entry resolves.
...
const deployHookedBorrowerOperationsImpl = async (deployerSigner) => {
    ...
    await impl.deployed();
    await deployments.save("BorrowerOperationsColFee", {
 ...
 *  zero-contracts-colfee @ private/colfee, claimCollWithFee two-leg split) and
 *  save the "CollSurplusPoolColFee" record the sipArgs Part 1 entry resolves —
...
const deployCollSurplusPoolImpl = async (deployerSigner) => {
    ...
    await impl.deployed();
    await deployments.save("CollSurplusPoolColFee", {
        address: impl.address,
...
/** Count occurrences of a ColFee event in a receipt (any emitting contract). */
const countColFeeEvents = (receipt, eventName) => {
    let count = 0;
