/**
 * SIP-0094 (ColFee Phase-1 activation, Part 1 + Part 2) — fork execution test.
 *
 ...
 * now run the test:
 *     __decryptionAlreadyDone__=TRUE npx hardhat test tests-onchain/colfee/colFeeActivationSips.test.js --network rskForkedMainnet
 */
...
describe("SIP-0094 ColFee Phase-1 activation (ownership-aggregated, GovernorOwner)", () => {
    const setupTest = createFixture(async ({ deployments }) => {
        ...
        await deployments.fixture(
            ["LoanTokenModules", "ProtocolModules", "ColFeeBorrowerExitOps"],
        ...
        // the audited zero-contracts-colfee commit (record
        // "BorrowerOperationsColFee").
        const stack = await deployColFeeStack(context.deployerSigner, RATE_BPS);
    ...
    it("executes both SIPs via governance, wires everything, charges live exits and leaves no-touch paths uncharged", async () => {
        if (!hre.network.tags["forked"]) {
            ...
            throw new Error(
                "colFeeActivationSips must run on a forked mainnet (rskForkedMainnet); " +
        ...
        // the ColFee pin kept LAST — the final governance action of the whole
        // activation (CF-1).
        ...
        // ── 2) ENABLE (ColFee Safe action — the test's deployer owns the
