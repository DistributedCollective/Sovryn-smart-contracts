...
contract("DevelopmentFund (Multisig Functions)", (accounts) => {
    ...
    it("Unlocked Token owner should not be able to approve a locked token owner, if not added by governance.", async () => {
        await expectRevert(
            developmentFund.approveLockedTokenOwner({from: multisig}),
            "No new locked owner added."
        );
    });
    ...
    it("Unlocked Token Owner should be able to withdraw part of the tokens after schedule.", async () => {
        ...
        await developmentFund.withdrawTokensByUnlockedTokenOwner(Math.floor(releaseTokenAmount[releaseTokenAmount.length - 1]/2), {from: multisig});
    });
    ...
    it("Unlocked Token Owner should be able to withdraw tokens after multiple schedule is passed.", async () => {
        ...
        await developmentFund.withdrawTokensByUnlockedTokenOwner(releaseTokenAmount[releaseTokenAmount.length - 1] + Math.floor(releaseTokenAmount[releaseTokenAmount.length - 2]/2), {from: multisig});
    });
    ...
    it("Unlocked Token Owner should not be able to withdraw tokens higher than the schedule.", async () => {
        ...
        assert.strictEqual(beforeTokenBalance.toNumber(), afterTokenBalance.toNumber() - releaseTokenAmount[releaseTokenAmount.length - 1], "Token amount not correct.")
    });
    ...
    it("Unlocked Token Owner should not be able to withdraw tokens without any duration is complete.", async () => {
        ...
        let value = randomValue();
        await expectRevert(
            developmentFund.withdrawTokensByUnlockedTokenOwner(value, {from: multisig}),
            "No release schedule reached."
        );
    });
