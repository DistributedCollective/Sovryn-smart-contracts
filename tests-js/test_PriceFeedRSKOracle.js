...
contract("PriceFeedRSKOracle", () => {
  ...
  beforeEach(async () => {
    ...
    priceFeedRSKOracle = await PriceFeedRSKOracle.new(
      priceFeedRSKOracleMockup.address
    );
  });
  ...
  it("should always return the current time for latestTimestamp", async () => {
    expect(
      await priceFeedRSKOracle.latestTimestamp.call()
    ).to.be.bignumber.equal(await latest());
