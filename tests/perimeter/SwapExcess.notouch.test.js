 ...
 *
 * Per `perimeter/docs/IMPLEMENTATION_DESIGN.md:301-309`, these refund paths
 ...
 *
 * Run:
 *   npx hardhat test tests/perimeter/SwapExcess.notouch.test.js
 */
...
contract("Perimeter — Swap-excess refund no-touch coverage (Phase 3 / Task 3.4)", (accounts) => {
    ...
    describe("_handleCollateralReturn excess-refund path (returnTokenIsCollateral=true)", () => {
        it("when the swap overfills the principal, the excess refund to the borrower is NOT Perimeter'd — only the main _finalizeSwapClose payout fires", async () => {
