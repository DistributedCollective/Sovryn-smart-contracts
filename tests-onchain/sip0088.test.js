...
describe("Protocol Modules Deployments and Upgrades via Governance", () => {
    ...
    const setupTest = createFixture(async ({ deployments }) => {
        ...
        if (
            loanOpenings.address ===
            (await sovrynProtocol.getTarget(modulesList.LoanOpenings.sampleFunction))
        ) {
            console.log("deploying LoanOpenings");
            deployLoanOpeningsResult = await deploy("LoanOpenings", {
                contract: "LoanOpenings",
                args: [],
            });

            await deployments.save("LoanOpenings", {
                address: deployLoanOpeningsResult.address,
                implementation: deployLoanOpeningsResult.address,
                abi: deployLoanOpeningsResult.abi,
                bytecode: deployLoanOpeningsResult.bytecode,
                deployedBytecode: deployLoanOpeningsResult.deployedBytecode,
