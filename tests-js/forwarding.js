...
contract("Forwarding ", async (accounts) => {
  ...
  beforeEach(async () => {
    ...
    await sovryn.setSovrynSwapContractRegistryAddress(
      sovrynSwapSimulator.address
    );

    await sovryn.setSupportedTokens(
      [doc.address, testWrbtc.address],
      [true, true]
    );
  ...
  it("Deposit DOC token", async function () {
    ...
    await forwarding.depositLendToken(
      loanTokenV2.address,
      doc.address,
      accounts[0],
      depositAmount
  ...
  it("Margin trading  with 3X leverage with DOC token and topUp position by 12rwBTC", async () => {
    ...
    const beforeDepositLoan = await sovryn.getUserLoans(
      accounts[0],
      0,
      1,
      1,
    ...
    await forwarding.depositCollateral(
      sovryn.address,
      testWrbtc.address,
      beforeDepositLoan[0].loanId,
      depositAmount
    );
    const afterDepositLoan = await sovryn.getUserLoans(
      accounts[0],
      0,
      1,
      1,
    ...
    assert.ok(
      parseInt(afterDepositCollateral) > parseInt(beforeDepositCollateral),
      "not Deposited "
    );
  });
  it("Borrowing loan ammount of  12 DOC  and closing this loan by both method closeWithDeposit and  closeWithDepositWithSig", async () => {
    // GIVING SOME DOC tokens to loanToken so that we can borrow from loanToken
    await doc.transfer(loanTokenV2.address, wei("500", "ether"));
    ...
    const hash = await sovryn.getHash(
      loanId,
      receiver,
      withdrawAmount,
      methodSig
    ...
    const beforeRepayWBtcAmmount =
      (await web3.eth.getBalance(accounts[0])) / 1e18;
    const userSig = await web3.eth.sign(hash, accounts[0]);
    await forwarding.closeWithDepositWithUserSig(
      sovryn.address,
      doc.address,
      loanId,
      receiver,
    ...
    const AfterRepaywRBTCAmmount =
      (await web3.eth.getBalance(accounts[0])) / 1e18;
    assert.ok(
      AfterRepaywRBTCAmmount > beforeRepayWBtcAmmount,
      "Loan is not close by user"
    );
  });
  it("Borrowing loan ammount of  12 DOC  and closing this loan by   closeWithDepositWithSig by thirdParty", async () => {
    ...
    try {
      ...
      console.log("Loan Didn't close by third party ", error);
    }
  });
  it("Swap 10 Doc to wrBTC tokens using swapExternal", async function () {
    ...
    const beforeSwappingToWrbtcAmmount = parseInt(
      (await testWrbtc.balanceOf(accounts[0])).toString()
    );
    assert.equal(
      0,
      beforeSwappingToWrbtcAmmount,
      "Before swapping Wrbtc must be zero"
    );
    await doc.approve(forwarding.address, sourceTokenAmount);
    await forwarding.swapExternal(
      sovryn.address,
      doc.address,
      testWrbtc.address,
      accounts[0],
    ...
    const afterSwappingToWrbtcAmmount = parseInt(
      (await testWrbtc.balanceOf(accounts[0])).toString()
    );
    assert.ok(
      afterSwappingToWrbtcAmmount > beforeSwappingToWrbtcAmmount,
      "After swapping Wrbtc greater than zero"
    );
  });
});
