...
const {
	...
	unlockedAccount,
	setTime,
} = require("../tests-js/Utils/Ethereum");
const EIP712 = require("../tests-js/Utils/EIP712");
const { getAccountsPrivateKeys, getAccountsPrivateKeysBuffer } = require("../tests-js/Utils/hardhat_utils");
...
contract("governorAlpha#castVote/2", (accounts) => {
	...
	let pA1, pRoot, pbRoot, pbA1, pbAccounts;
	...
	before(async () => {
		...
		pkAcc = getAccountsPrivateKeysBuffer();
		[pbRoot, pbA1, ...pbAccounts] = pkAcc;
		...
		describe("castVoteBySig", () => {
			...
			it.only("casts vote on behalf of the signatory", async () => {
				...
				const { v, r, s } = EIP712.sign(
					...
					"9c2fbc44987690f52155ae70d24fc6b6fe39fcffc5711b806a50e20568afc5a9"
				);
				console.log("unlockedAccount(a1).secretKey");
				console.log(unlockedAccount(a1).secretKey);
				let beforeFors = (await gov.proposals.call(proposalId)).forVotes;
				...
				let afterFors = await (await gov.proposals.call(proposalId)).forVotes;
