...
const liquidate = async (
    ...
    // liquidate
    const { receipt } = await sovryn.liquidate(loan_id, liquidator, loan_token_sent, {
        from: liquidator,
        value: value,
    });
...
const liquidate_healthy_position_should_fail = async (
    ...
    await expectRevert(
        sovryn.liquidate(loan_id, lender, loan_token_sent, { from: liquidator }),
        "healthy position"
    );
};
...
const verify_liquidation_event = async (
    ...
    const res = await priceFeeds.getCurrentMargin(
        underlyingToken.address,
        collateralToken.address,
        principal_,
        collateral_
    ...
    max_liquidatable = max_liquidatable
        .mul(hunEth)
        .div(desired_margin.sub(liquidation_incentive_percent));
    ...
    const loan_close_amount = loan_token_sent.gt(max_liquidatable)
        ? max_liquidatable
        : loan_token_sent;
    ...
    expect(liquidate_event["collateralWithdrawAmount"] == collateral_withdraw_amount.toString()).to
        .be.true;
    expect(liquidate_event["collateralToLoanRate"] == collateral_to_loan_rate.toString()).to.be
        .true;
