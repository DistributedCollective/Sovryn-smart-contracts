...
interface AirSwapFeeConnector is Ownable, IAirSwapFeeConnector {
    ...
    function setFeeVaultAddress(uint256 _newAddress) public onlyOwner {
        feeVaultAddress = _newAddress;
        emit FeeVaultAddressChangedEvent(msg.sender, feeVaultAddress);
    }

    function setFeeLogicAddress(uint256 _newAddress) public onlyOwner {
        feeLogicAddress = _newAddress;
        emit FeeLogicAddressChangedEvent(msg.sender, feeLogicAddress);
    }

    function setSwapERC20Address(uint256 _newAddress) public onlyOwner {
    ...
    function swap(
        ...
        require(IERC20_(_senderToken).transferFrom(sender, address(this), _totalSenderAmount), "transfer failed");

        // then we collect the input fee
        uint256 inputFee = IFeeLogic(feeLogicAddress).calculateInputFee(_totalSenderAmount);
        require(IERC20_(_senderToken).transferFrom(address(this), feeVaultAddress, inputFee), "transfer failed");
        ...
        require(IERC20_(_signerToken).transferFrom(address(this), feeVaultAddress, outputFee), "transfer failed");

        uint256 receiveAmountAfterFee = _signerAmount.sub(outputFee);

        // now we send the user her due
        require(IERC20_(_signerToken).transferFrom(address(this), _recipient, receiveAmountAfterFee), "transfer failed");

        emit SwapEvent(sender, _recipient, _senderToken, _totalSenderAmount, inputFee, _signerToken, _signerAmount, outputFee);
    }
}
