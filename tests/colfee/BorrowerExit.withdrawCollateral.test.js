 ...
 *
 *   1. ColFee globally disabled  → full gross to borrower, ExitFeeSkipped(INACTIVE).
 ...
 *
 * Run:
 *   npx hardhat test tests/colfee/BorrowerExit.withdrawCollateral.test.js
 */
...
contract("ColFee — borrower-exit withdrawCollateral (Phase 3 / Task 3.1)", (accounts) => {
    ...
    async function deploymentAndInitFixture() {
        ...
        // ── ColFee controller wired into the protocol, default OFF ─────────
    ...
    describe("Delegated-manager path (gate's `delegatedManagers[loanLocal.id][msg.sender]` arm)", () => {
        it("a delegated manager calling withdrawCollateral charges ColFee — proves the second arm of `isBorrowerExit`", async () => {
            await controller.setExitFeeEnabledTest(true);
    ...
    // The borrower-exit charge hook is a DEPLOYED CONTRACT (ColFeeBorrowerExitOps)
    // reached by delegatecall through a STORED pointer (set in the fixture via
    // sovryn.setColFeeBorrowerExitOps), not a link-time library. These pin the
    // new pointer's behavior: read-through, owner-gated rotation, and the
    // fail-open backstop when the pointer is mis-set.
    describe("ColFeeBorrowerExitOps pointer (the delegatecall charge hook)", () => {
        it("colFeeBorrowerExitOps() returns the pinned Ops address", async () => {
            const got = await sovryn.colFeeBorrowerExitOps();
        ...
        it("owner can rotate the Ops pointer; charging flows through the NEW hook", async () => {
            ...
            const newOps = await ColFeeBorrowerExitOps.new();
            const tx = await sovryn.setColFeeBorrowerExitOps(newOps.address, { from: owner });
            const ev = tx.logs.find((l) => l.event === "ColFeeBorrowerExitOpsSet");
            expect(ev, "ColFeeBorrowerExitOpsSet emitted").to.not.equal(undefined);
            expect(ev.args.current.toLowerCase()).to.equal(newOps.address.toLowerCase());
            expect((await sovryn.colFeeBorrowerExitOps()).toLowerCase()).to.equal(
        ...
        it("mis-set Ops pointer (a contract without the charge selector) → fail-open full gross", async () => {
            ...
            // forbids re-entering.
            await controller.setExitFeeEnabledTest(true);
            await sovryn.setColFeeBorrowerExitOps(SUSD.address, { from: owner });
        ...
        it("setColFeeBorrowerExitOps reverts for a non-owner", async () => {
            const newOps = await ColFeeBorrowerExitOps.new();
            await expectRevert(
                sovryn.setColFeeBorrowerExitOps(newOps.address, { from: account1 }),
                "unauthorized"
            );
        });

        it("setColFeeBorrowerExitOps(0) and (EOA) revert (EFC:not-contract)", async () => {
            await expectRevert(
                sovryn.setColFeeBorrowerExitOps(ZERO, { from: owner }),
                "EFC:not-contract"
            );
            await expectRevert(
                sovryn.setColFeeBorrowerExitOps(account1, { from: owner }),
                "EFC:not-contract"
            );
        });
    });
