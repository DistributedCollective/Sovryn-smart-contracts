...
/// Stateful invariant for the ColFee fee-leg (#4). A handler drives a random
...
contract OpsHost {
    bytes32 internal constant IMPL_SLOT = keccak256("test.colfee.opsImpl");
...
contract FeeLegHandler {
    ...
    constructor() public {
        ColFeeBorrowerExitOps ops = new ColFeeBorrowerExitOps();
        host = new OpsHost();
    ...
    function charge(
        ...
        uint256 toUser = ColFeeBorrowerExitOps(address(host)).chargeExitFeeAndPay(
            bytes32(0),
            address(this),
            USER,
            asset,
...
contract ColFeeFeeLegInvariant {
