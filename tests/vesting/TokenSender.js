...
contract("TokenSender", (accounts) => {
	...
	describe("transferSOV", () => {
		...
		it("only owner or admin should be able to transfer", async () => {
			await expectRevert(tokenSender.transferSOV(account1, 1000, { from: account1 }), "unauthorized");
		});

		it("fails if the 0 address is passed as receiver address", async () => {
			await expectRevert(tokenSender.transferSOV(ZERO_ADDRESS, 1000), "receiver address invalid");
		});
	...
	describe("transferSOVusingList", () => {
		...
		it("only owner or admin should be able to transfer", async () => {
			await expectRevert(tokenSender.transferSOVusingList([account1], [1000], { from: account1 }), "unauthorized");
		});

		it("fails if the 0 address is passed as receiver address", async () => {
			await expectRevert(tokenSender.transferSOVusingList([ZERO_ADDRESS], [1000]), "receiver address invalid");
		});
