...
describe("LockedSOVRewardTransferLogic", () => {
    ...
    describe("initialize", () => {
        it("fails if not an owner or admin", async () => {
            rewardTransferLogic = await LockedSOVRewardTransferLogic.new();
            await expectRevert(
                rewardTransferLogic.initialize(SOVToken.address, unlockedImmediatelyPercent, {
                    from: account1,
                }),
                "unauthorized"
            );
            ...
            await rewardTransferLogic.initialize(SOVToken.address, unlockedImmediatelyPercent, {
                from: account1,
            });
        });
        ...
        it("sets the expected values", async () => {
            ...
            let _unlockedImmediatelyPercent =
                await rewardTransferLogic.unlockedImmediatelyPercent();
    ...
    describe("changeLockedSOV", () => {
        it("fails if not an owner or admin", async () => {
            await expectRevert(
                rewardTransferLogic.changeLockedSOV(SOVToken.address, { from: account1 }),
                "unauthorized"
            );
        ...
        it("fails if invalid address", async () => {
            await rewardTransferLogic.addAdmin(account1);
            await expectRevert(
                rewardTransferLogic.changeLockedSOV(ZERO_ADDRESS, { from: account1 }),
                "Invalid address"
            );
        });
        ...
        it("should set a new LockedSOV", async () => {
            ...
            await rewardTransferLogic.addAdmin(account1);
            tx = await rewardTransferLogic.changeLockedSOV(newLockedSOV.address, {
                from: account1,
            });
    ...
    describe("changeUnlockedImmediatelyPercent", async () => {
        it("fails if not an owner or admin", async () => {
            await expectRevert(
                rewardTransferLogic.changeUnlockedImmediatelyPercent(unlockedImmediatelyPercent, {
                    from: account1,
                }),
                "unauthorized"
            );
            ...
            await rewardTransferLogic.addAdmin(account1);
            await rewardTransferLogic.changeUnlockedImmediatelyPercent(
        ...
        it("fails if invalid unlocked percent", async () => {
            await rewardTransferLogic.addAdmin(account1);
            await expectRevert(
                rewardTransferLogic.changeUnlockedImmediatelyPercent(new BN(10000), {
                    from: account1,
                }),
        ...
        it("should set a new unlocked percent", async () => {
            ...
            await rewardTransferLogic.addAdmin(account1);
            tx = await rewardTransferLogic.changeUnlockedImmediatelyPercent(newUnlockedPercent, {
                from: account1,
            });
    ...
    describe("transferReward", async () => {
        ...
        it("fails if account doesn't have reward tokens", async () => {
            await expectRevert(
                rewardTransferLogic.transferReward(account2, new BN(5), false, { from: account1 }),
                "invalid transfer"
            );
        });
        ...
        it("fails if invalid address to transfer", async () => {
            ...
            await SOVToken.approve(rewardTransferLogic.address, account1InitialBalance, {
                from: account1,
            });
            await expectRevert(
                rewardTransferLogic.transferReward(ZERO_ADDRESS, new BN(5), false, {
                    from: account1,
                }),
                "invalid transfer"
            );
        ...
        it("should account1 transfer reward to account2 without withdraw", async () => {
            ...
            await rewardTransferLogic.transferReward(account2, amountToTransfer, false, {
                from: account1,
            });
            let account1FinalBalance = await SOVToken.balanceOf(account1);
            expect(account1FinalBalance).bignumber.equal(
                account1InitialBalance.sub(amountToTransfer)
            );
        });
        ...
        it("should should account2 have locked and unlocked balance after transfer with withdraw", async () => {
            ...
            await rewardTransferLogic.transferReward(account2, amountToTransfer, true, {
                from: account1,
            });
            let lockedBalance = await lockedSOV.getLockedBalance(account2);
            let unlockedBalance = await lockedSOV.getUnlockedBalance(account2);
