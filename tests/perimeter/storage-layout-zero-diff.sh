...
# unstructured slots and the BorrowerExitPerimeter mixin declares no state vars.
...
#
# Usage:  tests/perimeter/storage-layout-zero-diff.sh [BASE_REF]
#   BASE_REF defaults to the pre-hooks commit.
set -u

REPO="$(cd "$(dirname "$0")/../.." && pwd)"
BASE_REF="${1:-51052975}"
WORKTREE="$(mktemp -d)/base-perimeter"
OUT="$(mktemp -d)"
