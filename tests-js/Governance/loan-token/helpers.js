...
const lend_btc_before_cashout = async (loanToken, total_deposit_amount, lender) => {
	const initial_balance = await lender.balance();
...
const cash_out_from_the_pool = async (loanToken, lender, underlyingToken, lendBTC) => {
	...
	if (lendBTC) {
		...
		balance_after_lending = await lender.balance();
	...
	if (lendBTC) {
		expect((await lender.balance()).gt(balance_after_lending)).to.be.true;
		expect((await lender.balance()).lt(initial_balance.sub(amount_withdrawn.mul(await loanToken.tokenPrice()).div(oneEth)))).to.be.true;
	} else {
