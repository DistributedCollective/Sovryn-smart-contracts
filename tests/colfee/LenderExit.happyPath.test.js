 ...
 * The `useLM` flag is orthogonal to ColFee: both branches converge on
 ...
 * pre-step (`_burnFromLM`) that doesn't touch the ColFee surface.
 *
 * Scenarios (from the colfee plan, Task 2.2 Step 1):
 *   1. ColFee globally disabled  → ExitFeeSkipped(reason=INACTIVE, rate=0),
 ...
 *
 * Run:
 *   npx hardhat test tests/colfee/LenderExit.happyPath.test.js
 */
...
contract("ColFee — lender-exit happy path (Phase 2 / Task 2.2)", (accounts) => {
    ...
    async function fixture() {
        ...
        // ── ColFee controller wired into both iTokens, default OFF ─────────
    ...
    describe("Pool-balance invariant — no fee residue", () => {
        it("iSUSD's underlying balance decrements by EXACTLY gross (fee leaves the pool)", async () => {
            ...
            // in the colfee plan: no value sticks in the iToken contract.
            expect(
