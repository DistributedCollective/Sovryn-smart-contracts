...
//const FeesHelper = artifacts.require("MockLoanTokenLogic");
...
contract("Affiliates", (accounts) => {
	...
	beforeEach(async () => {
		...
		//await sovryn.replaceContract((await MockLoanTokenLogic.new()).address);
	...
	it("User Margin Trade with Affiliate runs correctly", async () => {
		...
		const receipt = await loanTokenV2.marginTradeAffiliate(
	...
	it("Affiliates Referrer withdraw fees works correctly", async () => {
		const leverageAmount = web3.utils.toWei("3", "ether");
		//loan tokens sent to iToken contract to start Margin Trading
		const loanTokenSent = web3.utils.toWei("20", "ether");
		// AUDIT: should the call be allowed from arbitrary address to set an affiliate in
		...
		//SUCCESS
		//partial withdraw
		await sovryn.withdrawAffiliatesReferrerTokenFees(doc.address, referrer, referrerFee.divn(2), { from: referrer });
		const newReferrerFee = await sovryn.affiliatesReferrerBalances(referrer, doc.address);
		expect(newReferrerFee, "Incorrect partial balance after half withdraw").to.be.bignumber.equal(referrerFee.divn(2));

		//complete withdraw
		await sovryn.withdrawAffiliatesReferrerTokenFees(doc.address, referrer, newReferrerFee, { from: referrer });
		expect(await doc.balanceOf(referrer), "Incorrect withdraw amount").to.be.bignumber.equal(referrerFee);
		expect(
	...
	it("Only the first trade users can be assigned Affiliates Referrer", async () => {
		await loanTokenV2.setUserNotFirstTradeFlag(trader); // can be called only from loan tokens pool addresses
		await loanTokenV2.setAffiliatesReferrer(trader, referrer); // can be called only from loan tokens pool addresses
