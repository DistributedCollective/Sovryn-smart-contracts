...
//
// The Perimeter lending-side hook adds NO storage to any deployed upgradeable
// contract: the exit-fee controller pointer and its siblings live in
// EIP-1967-style unstructured slots (contracts/utils/PerimeterLib.sol) and no new
...
// captured from the pre-Perimeter base ref 83871cd0 — the layout the LIVE mainnet
...
//
// RUN:
//   __decryptionAlreadyDone__=TRUE npx hardhat test tests/perimeter/StorageLayout.zerodiff.test.js
//
...
//      tests/perimeter/baselines/storage-layout.pre-perimeter-base.json, updating
...
const BASELINE = path.join(__dirname, "baselines", "storage-layout.pre-perimeter-base.json");
...
const TARGETS = [
    ...
    "contracts/modules/ExitFeeModule.sol:ExitFeeModule", // Perimeter addition — must add no state
    "contracts/utils/BorrowerExitPerimeterOps.sol:BorrowerExitPerimeterOps", // Perimeter addition, delegatecalled in protocol context
...
describe("Perimeter — storage-layout zero-diff (lending security perimeter)", () => {
    let baseline;
    ...
    for (const fq of TARGETS) {
        it(`${fq}: current layout == pre-Perimeter baseline (no appended state)`, async () => {
            ...
            assert.deepStrictEqual(
                current,
                base,
                `STORAGE LAYOUT DIFF for ${fq} vs pre-Perimeter baseline:\n` +
