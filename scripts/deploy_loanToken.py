...
'''
def main():
    wbtcAddress = '0xc90bB9fEee164263709336C7e5E9F8e540fA3C6D'
    ...
    tokens = Munch()
    tokens.wbtc = Contract.from_abi("TestWbtc", address = wbtcAddress, abi = TestToken.abi, owner = acct)
...
def deployLoanTokens(acct, sovryn, tokens):
    ...
    print('\n DEPLOYING ISUSD')
    contract = deployLoanToken(acct, sovryn, tokens.susd.address, "SUSD", "SUSD", tokens.rbtc.address, tokens.wbtc.address)
    ...
    print('\n DEPLOYING IRBTC')
    contract = deployLoanToken(acct, sovryn, tokens.rbtc.address, "RBTC", "RBTC", tokens.susd.address, tokens.wbtc.address)
    print("initializing the lending pool with some tokens, so we do not run out of funds")
...
def deployLoanToken(acct, sovryn, loanTokenAddress, loanTokenSymbol, loanTokenName, collateralAddress, wbtcAddress):
    ...
    loanToken = acct.deploy(LoanToken, loanTokenLogic.address, sovryn.address, wbtcAddress)
