 ...
 *     `PerimeterLib.safeControllerLookup`), the delay quote resolves to `d == 0`
 ...
 *     (`PERIMETER:delay-quote-failed`), so an active perimeter can never be
 *     silently bypassed.
 *
 * Run:
 *   npx hardhat test tests/perimeter/LenderExit.controllerPointer.failopen.test.js
 */
...
contract("Perimeter delay — controller-pointer fail-open / quote fail-closed", (accounts) => {
    ...
    describe("controller pinned + delay quote REVERTS — fail-CLOSED (exit reverts)", () => {
        it("reverts the whole burn with PERIMETER:delay-quote-failed", async () => {
