...
contract ColFeeBorrowerExit is ModuleCommonFunctionalities, VaultController {
    ...
    ///         and record — runs in the deployed `ColFeeBorrowerExitOps` via
    ...
    function _maybeDelayBorrowerExit(
        ...
        (uint32 d, address effOrig, address effOwner) = ColFeeLib.safeQuoteDelay(
        ...
        (bool ok, bytes memory ret) = ColFeeLib.getBorrowerExitOps().delegatecall(
            abi.encodeWithSelector(
                ColFeeBorrowerExitOps(address(0)).escrowBorrowerExit.selector,
                loanLocal.lender,
                assetToken,
                receiver,
                toUser,
        ...
        require(ret.length == 32, "COLFEE:exit-ops-unset");
