...
contract("LoanTokenLending", (accounts) => {
    ...
    describe("Test lending using TestToken", () => {
        ...
        it("Should revert when calling disableLoanParams by other than Admin", async () => {
            await expectRevert(
                loanToken.disableLoanParams([WRBTC.address], [false], { from: account2 }),
                "unauthorized"
            );
        });
        ...
        it("Should revert when calling disableLoanParams w/ mismatching parameters", async () => {
            await expectRevert(
                loanToken.disableLoanParams([WRBTC.address, ZERO_ADDRESS], [false]),
                "count mismatch"
            );
        });
        ...
        it("test profit", async () => {
            ...
            await SUSD.approve(
                loanToken.address,
                balanceOf0.add(new BN(wei("100", "ether"))).toString()
            );
