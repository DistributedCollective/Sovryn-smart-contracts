...
task("bridge:getMaxPerToken", "Get bridge max tokens limit per transfer for a token")
    .addPositionalParam("token", "Address or deployment name (e.g. DLLR) of a token")
    .addOptionalParam("bridge", "Bridge contract to get token max limit per transfer", "Bridge")
    .setAction(async ({ token, bridge }, hre) => {
        const {
            deployments: { get, getArtifact },
            ethers,
        } = hre;
        ...
        const maxPerToken = await allowTokensContract.getMaxPerToken(tokenAddress);

        logger.info(
            `The bridge ${bridgeContract.address} max tokens allowed per transfer for token ${tokenAddress} is ${ethers.utils.formatEther(maxPerToken)}`
        );
    });
...
task("bridge:setMaxPerToken", "Set max tokens transfer limit per transfer for a token")
    .addPositionalParam("token", "Address or deployment name (e.g. DLLR) of a token")
    .addPositionalParam(
        "maxAmount",
        "Max token single transfer limit amount to set in ETH (will be multiplied by 1e18)"
    )
    .addOptionalParam("bridge", "Bridge contract to set token max limit on", "Bridge")
    .addOptionalParam("signer", "Signer name: 'signer' or 'deployer'", "deployer")
    .setAction(async ({ signer, token, maxAmount, bridge }, hre) => {
        const {
            deployments: { get, getArtifact },
            ethers,
        } = hre;
        ...
        const allowTokensInterface = new ethers.utils.Interface(allowTokensArtifact.abi);
        let data = allowTokensInterface.encodeFunctionData("setMaxPerToken", [
            tokenAddress,
