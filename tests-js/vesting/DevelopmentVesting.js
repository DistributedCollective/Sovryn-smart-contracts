...
contract('DevelopmentVesting:', accounts => {
    ...
    describe('withdrawTokens:', () => {
        ...
        it('fails if amount is 0', async () => {
            await expectRevert(vesting.withdrawTokens(0, root),
                "amount needs to be bigger than 0");
        });

        it('fails if amount iss not available', async () => {
            await expectRevert(vesting.withdrawTokens(100, root),
        ...
        it('fails if receiver address invalid', async () => {
            ...
            await expectRevert(vesting.withdrawTokens(amount, ZERO_ADDRESS),
    ...
    describe('changeSchedule:', () => {
        ...
        it('fails if the vesting duration is shorter than the cliff', async () => {
            await expectRevert(vesting.changeSchedule(100, 99, 100),
                "duration must be bigger than or equal to the cliff");
        });

        it('fails if the vesting duration is shorter than the frequency', async () => {
            await expectRevert(vesting.changeSchedule(10, 100, 200),
                "frequency is bigger than (duration - cliff)");
        });
    ...
    describe('vestTokens:', () => {
        ...
        it('fails if amount is 0', async () => {
            await expectRevert(vesting.vestTokens(0),
                "amount needs to be bigger than 0");
        });

        it('fails if transfer fails', async () => {
            await expectRevert(vesting.vestTokens(12345),
                "invalid transfer");
        });
    ...
    describe('withdrawByAllSchedules:', () => {
        ...
        it('fails if receiver address invalid', async () => {
            await expectRevert(vesting.withdrawByAllSchedules(ZERO_ADDRESS),
                "receiver address invalid");
        });

        it('fails if no available tokens', async () => {
            await expectRevert(vesting.withdrawByAllSchedules(root),
                "no available tokens");
        });
