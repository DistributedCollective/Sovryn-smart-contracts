...
import "../../contracts/utils/ColFeeLib.sol";
import "../../contracts/utils/ColFeeBorrowerExitOps.sol";
import "../../contracts/interfaces/colfee/IExitFeeController.sol";

/// Foundry fuzz coverage for the ColFee amount split — the trust boundary with
...
contract ColFeeSplitFuzzTest {
    ...
    function testFuzz_validImpliesConservation(
        ...
        if (ColFeeLib.quoteIsValid(q, gross)) {
    ...
    function testFuzz_conservationImpliesValid(
        ...
        q.feeReceiver = feeReceiver;
        require(ColFeeLib.quoteIsValid(q, gross), "c:should-be-valid");
    }
    ...
    function testFuzz_safeQuoteNeverReverts(bytes memory ret, uint256 gross) public {
        ...
        IExitFeeController.ExitFeeQuote memory q = ColFeeLib.safeQuote(
