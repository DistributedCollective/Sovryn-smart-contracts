/**
 * `LoanClosingsShared._handleLoanTokenReturn` excess-collateral refund is
 * Perimeter-charged on a voluntary close, the same as every other exit route.
 *
 * The route in question: `closeWithSwap` with
 ...
 * That payout routes through `_withdrawAssetChargingExitFee` with the same
 * `_exitFeeChargeable(origin, loan)` gate as `_finalizeSwapClose` — charged
 * only on `CloseOrigin.VoluntaryClose` — matching every equivalent route
 * (`closeWithDeposit`, `returnTokenIsCollateral=true`, `withdrawCollateral`),
 * so a borrower cannot extract the whole position equity fee-free through
 * this path.
 *
 * Scenarios:
 *
 ...
 *
 * Run:
 *   npx hardhat test tests/perimeter/BorrowerExit.loanTokenReturn.test.js
 */
...
contract(
    "Perimeter — the excess-collateral refund is charged on a voluntary close",
    (accounts) => {
        ...
        /// so the excess-collateral branch is reached: swapAmount ≈
        /// principal/price (+10% buffer for swap fees) so the partial swap
        /// covers the full principal and `_handleLoanTokenReturn` pays out
        /// `collateral - swapAmount` as excess.
        ...
        describe("voluntary close, controller active at 25 bps", () => {
            it("charges the excess-collateral leg: ExitFeeApplied, feeReceiver delta == fee, borrower gets net", async () => {
                ...
                expect(collateralLeg.length, "exactly one charge on the collateral leg").to.equal(
                    1
                );
                ...
                // The charged gross is precisely the unswapped collateral that
                // would otherwise leak to the borrower uncharged.
                ...
                expect(residualLeg[0].args.recipient.toLowerCase()).to.equal(
                    account1.toLowerCase()
                );
            });
        });
        ...
        describe("2. Fail-open preserved on the new leg (controller disabled)", () => {
            it("pays the full excess collateral to the borrower; ExitFeeSkipped(INACTIVE)", async () => {
                ...
                expect(skippedCollateral.length, "collateral leg went through the hook").to.equal(
                    1
                );
        ...
        describe("3. Rollover through _handleLoanTokenReturn stays uncharged (origin gate)", () => {
            it("tiny-position force-close via rollover emits no Perimeter events with the controller fully active", async () => {
