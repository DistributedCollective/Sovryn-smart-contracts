...
contract LiquidityMiningV2 is ILiquidityMiningV2, LiquidityMiningStorageV2 {
    ...
    function addRewardToken(
        ...
        require(
            _rewardToken == rewardTransferLogic.getRewardTokenAddress(),
            "Reward token and transfer logic mismatch"
        );
    ...
    function add(
        ...
        require(
            _rewardTokens.length == _allocationPoints.length,
        ...
        poolInfoList.push(
            PoolInfo({ poolToken: IERC20(_poolToken), rewardTokens: _rewardTokens })
        );
        ...
        for (uint256 i = 0; i < _rewardTokens.length; i++) {
            _associatePoolToRewardToken(_poolToken, _rewardTokens[i], _allocationPoints[i]);
        }
    ...
    /**
     * @notice adds reward tokens to an existing pool. Can only be called by the owner or an admin.
     * @param _poolToken the address of pool token
     * @param _rewardTokens the addresses of reward tokens for given pool
     * @param _allocationPoints the allocation points (weight) for the given pool and each reward token
     */
    function addTokensToExistingPool(
        address _poolToken,
        address[] calldata _rewardTokens,
        uint96[] calldata _allocationPoints,
        bool _withUpdate
        ...
        uint256 _poolId = _getPoolId(_poolToken);
    ...
    /**
     * @notice associate Pool to RewardToken. Can only be called by the owner or an admin
     * @param _poolToken the address of pool token
     * @param _rewardToken the addresses of reward tokens for given pool
     * @param _allocationPoint the allocation points (weight) for the given pool and each reward token
     */
    function _associatePoolToRewardToken(
        address _poolToken,
        address _rewardToken,
        ...
        require(
            poolInfoRewardTokensMap[poolId][_rewardToken].allocationPoint == 0,
            "Already associated"
        );
    ...
    function _getUserAccumulatedReward(
        ...
        return
            user.amount.mul(accumulatedRewardPerShare).div(PRECISION).sub(
                user.rewards[_rewardToken].rewardDebt
            );
    }
    ...
    function _updateRewardDebt(
        ...
        user.rewards[rewardToken].rewardDebt = user
            .amount
            .mul(poolInfoRewardTokensMap[poolId][rewardToken].accumulatedRewardPerShare)
            .div(PRECISION);
    }
    ...
    function _transferReward(
        ...
        if (balance >= userAccumulatedReward) {
            rewardToken.totalUsersBalance = rewardToken.totalUsersBalance.sub(
            ...
            require(
                token.approve(transferLogic.senderToAuthorize(), userAccumulatedReward),
                "Approve failed"
            );
     ...
     * @param _poolToken the address of pool token
     */
    function getPoolRewards(address _poolToken)
        external
        view
        returns (PoolInfoRewardToken[] memory)
    {
        uint256 poolId = _getPoolId(_poolToken);
     ...
     * @param _rewardToken the address of reward token
     */
    function getPoolReward(address _poolToken, address _rewardToken)
        external
        view
        returns (PoolInfoRewardToken memory)
    {
    ...
    function getUserBalanceList(address _user) external view returns (UserBalance[][] memory) {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            for (uint256 j = 0; j < rewardLength; j++) {
                ...
                userBalanceList[i][j].accumulatedReward = _getUserAccumulatedReward(
                    i,
                    _rewardToken,
                    _user
                );
     ...
     * @param _user the address of the user
     */
    function getUserInfo(address _poolToken, address _user)
        external
        view
        returns (PoolUserInfo memory)
    {
        uint256 poolId = _getPoolId(_poolToken);
    ...
    function _getPoolUserInfo(uint256 _poolId, address _user)
        internal
        view
        returns (PoolUserInfo memory)
    {
     ...
     * @param _user the address of the user
     */
    function getUserAccumulatedRewardList(address _user)
        external
        view
        returns (UserAccumulatedReward[][] memory)
    {
        uint256 length = poolInfoList.length;
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            for (uint256 j = 0; j < rewardsLength; j++) {
                ...
                rewardList[i][j].accumulatedReward = _getUserAccumulatedReward(
                    i,
                    poolInfo.rewardTokens[j],
                    _user
                );
            }
        }
