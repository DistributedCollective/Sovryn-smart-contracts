/**
 * ColFee EXECUTION-DAY DRESS RUN — the activation rehearsed against a stack
 * deployed by the REAL deploy pipeline, not fixtures.
 *
 * Prerequisites (all on the same still-running fork node):
 *   Phase A: colfee repo forge scripts 01→04 (full runbook env, 10 bps,
 ...
 *            ColFeeBorrowerExitOps,ProtocolModules` and the two Zero
 ...
 *
 * Gate: set COLFEE_DRESS_RUN=TRUE (skipped otherwise — this spec is
 * meaningless without the pre-deployed stack).
 *
 * It executes all three proposals through real governance (the same
 * `sips:create` task and builders execution day uses — including the
 * per-governor proposer-reuse shape: ONE proposer account creates Part 1,
 * Part 2 after Part 1 queues, and Part 3 on GovernorAdmin) and asserts:
 ...
 *   - live charged flows on both products once the ColFee Safe (the test's
 ...
 *       npx hardhat test tests-onchain/colfee/colFeeExecutionDayDressRun.test.js --network rskForkedMainnet
 */
...
} = require("./colFeeSipTestHelpers");

const { ethers, deployments } = hre;
const { get } = deployments;

// The real PriceFeeds.queryRate(XUSD, WRBTC) near the pinned fork block. The
// production WRBTC feed (PriceFeedsMoC) reads a MoC medianizer whose values
// expire, and the fallback oracle was deactivated by SIP-0084 — so on a fork,
// live-feed queries break as soon as blocks advance past the pin. The lending
// borrower flows therefore run against a PriceFeedsLocal seeded with this
// pinned real rate (same tolerance note as in colFeeActivationSips.test.js).
...
// The Exchequer Multisig — owner + operational admin of both ColFee proxies
...
const DRESS_RUN = process.env.COLFEE_DRESS_RUN === "TRUE";

(DRESS_RUN ? describe : describe.skip)(
    "ColFee execution-day dress run (live-deployed stack)",
    () => {
        ...
        const setupTest = async () => {
            ...
            // which otherwise win over fork-local files in this repo's
            // external-deployments config (and deployments.fixture would then
            // silently redeploy fresh copies, defeating the dress run). No
            // deploy scripts run here at all.
            const fs = require("fs");
            ...
            expect(await controller.owner(), "controller owner = Exchequer").to.equal(safeAddress);
            expect(await controller.admin(), "controller admin = Exchequer").to.equal(safeAddress);
            expect(await vault.owner(), "vault owner = Exchequer").to.equal(safeAddress);
            expect(await vault.admin(), "vault admin = Exchequer").to.equal(safeAddress);
            expect(await vault.defaultRecipient(), "vault sweeps to the Safe").to.equal(
                safeAddress
            );
            expect(await controller.feeReceiver(), "feeReceiver = vault proxy").to.equal(
                vault.address
            );
            ...
            for (const surfaceId of [
                ...
                expect(policy.rateBps, `surface ${surfaceId} at launch rate`).to.equal(RATE_BPS);
            }

            // The Safe signs the operational legs (enable + kill switch) below.
            const safeSigner = await getImpersonatedSigner(safeAddress);
            await setBalance(safeAddress, ONE_RBTC.mul(10));

            return { context, stack, hookedImpl, poolImpl, safeSigner };
        };
        ...
        it("executes both SIPs via governance, wires everything, charges live exits and leaves no-touch paths uncharged", async () => {
            if (!hre.network.tags["forked"]) {
                ...
                throw new Error(
                    "the dress run must run on a forked mainnet (rskForkedMainnet); " +
            ...
            const xusd = new ethers.Contract(
            ...
            const preColFeeZeroImpl = await boProxy.getImplementation();
            ...
            const preColFeePoolImpl = await collSurplusPoolProxy.getImplementation();
            ...
            const preColFeeLiquidationTarget = await protocol.getTarget(
            ...
            // Part 3 on GovernorAdmin, SAME proposer account: the one-live-
            // proposal limit is per-governor, so the GovernorAdmin run reuses an
            // Owner proposer — exactly the execution-day P8 shape.
            const { proposalId: sip0094Part3Id } = await createAndQueueSip(
                ctx,
                "getArgsSip0094Part3",
                "governorAdmin"
            );
            ...
            // Treasury leg: ONE fund withdrawal (Adoption) then one forwarding
            // transfer, with the fee-controller pin kept LAST — the final
            // governance action of the whole activation (CF-1). The Development
            // Fund is deliberately absent: its lockedTokenOwner is the Exchequer
            // Multisig, which sweeps it by a companion multisig transaction
            // (SIP-0094 text, 2026-08-11) — asserted untouched below.
            const sovToken = new ethers.Contract(
            ...
            expect(
                part2Signatures[2],
                "the fee-controller pin must stay the LAST action"
            ).to.equal("setExitFeeController(address)");
            for (const target of part2Targets) {
                expect(
                    addr(target),
                    "the Development Fund must NOT be a Part 2 target"
                ).to.not.equal(devFundAddress);
            }
            ...
            // Values, not shapes: the amount is the Adoption Fund's entire live
            // SOV balance, it is non-zero, and the forwarding transfer moves
            // exactly that amount to the Exchequer — no dust on the Timelock.
            ...
            expect(await boProxy.getImplementation()).to.equal(preColFeeZeroImpl);
            expect(await collSurplusPoolProxy.getImplementation()).to.equal(preColFeePoolImpl);
            ...
            // sweep is all-or-nothing with the ColFee pin. No SOV moved anywhere.
            ...
            ).to.equal(preColFeeLiquidationTarget);
            ...
            expect(await protocol.colFeeBorrowerExitOps()).to.equal(opsDeployment.address);
            expect(await protocol.exitFeeController()).to.equal(ethers.constants.AddressZero);
            expect(countColFeeEvents(part1Receipt, "ColFeeBorrowerExitOpsSet")).to.equal(1);
            ...
            expect(await collSurplusPoolProxy.getImplementation()).to.not.equal(preColFeePoolImpl);
            expect(await boProxy.getImplementation()).to.equal(hookedImpl.address);
            expect(await boProxy.getImplementation()).to.not.equal(preColFeeZeroImpl);
            ...
            expect(countColFeeEvents(part1Receipt, "ExitFeeControllerSet")).to.equal(1);
            ...
            expect(countColFeeEvents(receipt, "ExitFeeApplied")).to.equal(0);
            ...
            // own withdrawal, the Exchequer gains exactly that amount, the
            // Timelock — which held the tokens only mid-transaction — ends where
            // it started, and the Development Fund is NOT touched by governance
            // (its sweep is the companion Exchequer multisig transaction).
            expect(
            ...
            expect(countColFeeEvents(part2Receipt, "ExitFeeControllerSet")).to.equal(1);
            ...
            // ── Part 3 executes NOW (GovernorAdmin, 1-day timelock, CP-D3) ──
            // Its cycle is independent of the ColFee parts and shorter, so on
            // execution day it lands around them; in this spec it MUST land
            // before the no-touch sections' month-long time jumps push it past
            // the Timelock grace period — a queued proposal that outlives the
            // grace period EXPIRES (runbook Phase D note 6; this dress run's
            // first attempt hit exactly that). setAPR runs _issueSOV, which
            // reads ZeroCommunityIssuance's OWN feed — swap it to the local
            // feed first (MoC-stale on a fork), via its impersonated owner.
            ...
            expect(await communityIssuance.APR(), "subsidy APR live before Part 3").to.equal(500);
            await executeQueuedSip(ctx, sip0094Part3Id, "governorAdmin");
            expect(await communityIssuance.APR(), "subsidy APR off after Part 3").to.equal(0);
            ...
            const funder = await getImpersonatedSigner(
                "0x00000000000000000000000000000000c01fee11"
            );
            const closer = await getImpersonatedSigner(
                "0x00000000000000000000000000000000c01fee12"
            );
            ...
            expect(applied.feeAmount.gt(0), "collateral-side residual actually charged").to.be
                .true;
            expect(applied.feeAmount).to.equal(applied.grossAmount.mul(RATE_BPS).div(TEN_K));
            expect(applied.grossAmount).to.equal(applied.feeAmount.add(applied.netAmount));
            expect(await ethers.provider.getBalance(colFee.vault.address)).to.equal(
            ...
            const keeper = await getImpersonatedSigner(
                "0x00000000000000000000000000000000c01fee01"
            );
            ...
            expect(countColFeeEvents(receipt, "ExitFeeApplied")).to.equal(0);
            expect(countColFeeEvents(receipt, "ExitFeeSkipped")).to.equal(0);
            expect(await ethers.provider.getBalance(colFee.vault.address)).to.equal(
                vaultRbtcBefore
            );
            ...
            // and principal repaid. (A ColFee regression that bricked liquidation
            ...
            //     issuance was re-pointed to the local feed when Part 3
            ...
            receipt = await (
                await stabilityPool
                    .connect(closer)
                    .provideToSP(spAmount, ethers.constants.AddressZero)
            ).wait();
            expect(countColFeeEvents(receipt, "ExitFeeApplied")).to.equal(0);
