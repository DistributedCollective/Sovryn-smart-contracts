...
contract("governorAlpha#castVote/2", (accounts) => {
    ...
    describe("We must revert if:", () => {
        it("There does not exist a proposal with matching proposal id where the current block number is between the proposal's start block (exclusive) and end block (inclusive)", async () => {
            await expectRevert(
                gov.castVote.call(proposalId, true),
    ...
    describe("Otherwise", () => {
        ...
        describe("and we take the balance returned by GetPriorVotes for the given sender and the proposal's start block, which may be zero,", () => {
            ...
            it("and we add that ForVotes", async () => {
                ...
                let expectedVotes = await staking.getPriorVotes.call(
                    actor,
                    proposal.startBlock.toString(),
                    proposal.startTime.toString()
                );
                expect(new BigNumber(afterFors).toString()).to.be.equal(
                    new BigNumber(expectedVotes.toString()).toString()
                );
            });
            ...
            it("or AgainstVotes corresponding to the caller's support flag.", async () => {
                ...
                expect(new BigNumber(afterAgainsts).toString()).to.be.equal(
                    new BigNumber(expectedVotes.toString()).toString()
                );
            });
        });
        ...
        it("receipt uses one load", async () => {
            ...
            let expectedVotes2 = await staking.getPriorVotes.call(
                actor2,
                proposal.startBlock.toString(),
                proposal.startTime.toString()
            );

            expect(new BigNumber(trxReceipt.votes.toString()).toString()).to.be.equal(
                new BigNumber(expectedVotes.toString()).toString()
            );
            expect(trxReceipt.hasVoted).to.be.equal(true);
            expect(trxReceipt.support).to.be.equal(true);

            expect(new BigNumber(trxReceipt2.votes.toString()).toString()).to.be.equal(
                new BigNumber(expectedVotes2.toString()).toString()
            );
    ...
    describe("Check votes for a proposal creator:", () => {
        it("compare votes", async () => {
            ...
            await staking.stake(amount, kickoffTS.add(new BN(TWO_WEEKS)), actor, actor, {
                from: actor,
            });
