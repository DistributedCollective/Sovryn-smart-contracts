...
const func = async function (hre) {
    ...
    const multisigAddress = (await get("ContractsGuardianMultisig")).address;
    ...
    if (!(await osSOV.hasRole(AUTHORISED_MINTER_ROLE, stakingRewards.address))) {
        ...
        if ((await osSOV.owner()) === multisigAddress) {
            ...
            // await sendWithMultisig(multisigAddress, osSOV.address, data, deployer); //@todo adapt for Safe
            //log(col.yellow("This multisig tx requires signatures and execution"));
            log(col.yellow("data for Safe tx:"));
            log(col.blueBright(data));
            log(col.yellow("Create and execute Safe tx"));
        } else {
