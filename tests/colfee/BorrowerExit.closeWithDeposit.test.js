 ...
 *
 * Run:
 *   npx hardhat test tests/colfee/BorrowerExit.closeWithDeposit.test.js
 */
...
contract("ColFee — borrower-exit closeWithDeposit (Phase 3 / Task 3.2)", (accounts) => {
    ...
    describe("Senior accounting (lender repayment + interest) unchanged by ColFee", () => {
        it("the residual that ColFee charges is collateral only — lender's loanToken account is untouched by the fee leg", async () => {
            await controller.setExitFeeEnabledTest(true);
            ...
            // lender's SUSD balance accounting must be untouched by ColFee.
