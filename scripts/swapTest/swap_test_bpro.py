...
with open('./scripts/swapTest/swap_test_bpro.json') as config_file:
...
def setup():
    global sovryn, loan_token, loan_token_address, BPro, RBTC, loan_token_settings
...
def test_loan_address():
    ...
    if loan_token_address != BPro.address:
...
def test_margin_trading_sending_collateral_tokens():
    ...
    collateral_token_sent = sovryn.getRequiredCollateral(BPro.address,RBTC.address,loan_token_sent*2,50e18, False)
...
def test_margin_trading_sending_loan_tokens():
    ...
    loan_token_after_bpro_balance = BPro.balanceOf(loan_token_address)
    ...
    if loan['loanToken'] != BPro.address:
...
def test_lend_to_the_pool():
    ...
    initial_balance = BPro.balanceOf(lender)

    BPro.approve(loan_token.address, total_deposit_amount)

    if BPro.balanceOf(lender) != initial_balance:
        raise Exception("Failed to validate `test_lend_to_the_pool` - balance is incorrect")

    loan_token.mint(lender, deposit_amount)
    if BPro.balanceOf(lender) != initial_balance - deposit_amount:
        raise Exception("Failed to validate `test_lend_to_the_pool` - balance is incorrect")
...
def test_cash_out_from_the_pool():
    ...
    if BPro.balanceOf(lender) < initial_balance - amount_withdrawn * loan_token.tokenPrice() / 1e18:
...
def test_supply_interest_fee():
    BPro.approve(loan_token.address,1e17)
    loan_token.mint(acct, 1e16)
...
def initialize_test_transfer(BPro, accounts, _loan_token):
    sender = accounts[0]
    receiver = accounts[1]
    amount_to_buy = 100e18
    BPro.approve(_loan_token.address, amount_to_buy)
...
def test_transfer():
    amount_sent, receiver, sender = initialize_test_transfer(BPro, accounts, loan_token)
...
def test_liquidate():
    ...
    loan_token.mint(lender, 1e16)
    loan_token_sent = 1e16
    BPro.mint(borrower, loan_token_sent)
    BPro.mint(liquidator, loan_token_sent)

    BPro.approve(loan_token.address, loan_token_sent, {'from': borrower})
