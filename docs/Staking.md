...
```javascript
nction setNewStakingContract(address _newStakingContract) public onlyOwner {
...
```javascript
nction setFeeSharing(address _feeSharing) public onlyOwner {
...
```javascript
nction setWeightScaling(uint96 _weightScaling) public onlyOwner {
		require(
...
```javascript
nction migrateToNewStakingContract() public {
...
```javascript
nction unlockAllTokens() public onlyOwner {
		allUnlocked = true;
...
```javascript
nction getStakes(address account) public view returns (uint256[] memory dates, uint96[] memory stakes) {
		uint256 latest = timestampToLockDate(block.timestamp + MAX_DURATION);
