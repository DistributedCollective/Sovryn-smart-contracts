...
const getArgsSip0094Part1 = async (hre) => {
    ...
    const args = {
        ...
        data: datas,
        description:
            "SIP-0094 (Part 1): Perimeter Fee Activation and Adoption Fund Transfer — 1 of 3 executable parts (GovernorOwner). Executes the 10 Perimeter Fee installation actions: registers the exit-fee-hooked LM and WrbtcLM iToken beacon modules (2), replaces the LoanClosingsRollover, LoanClosingsWith and LoanMaintenance protocol modules (3), registers the ExitFeeModule admin module (1), sets BorrowerExitPerimeterOps (1), upgrades the Zero CollSurplusPool implementation (1), then upgrades the Zero BorrowerOperations implementation and wires its exit-fee controller in the same atomic transaction (2). Fee charging stays globally disabled throughout. Details: https://github.com/DistributedCollective/SIPS/blob/3fdc85f336359fa6575d8e629514eef0d6ddeec3/SIP-0094.md, sha256: 8a54a1d2ec447e6e609017940742f8a6bedce1ff0f72bb58d024f6c5f42c712d",
    };
...
const getArgsSipPerimeterPart2 = async (hre) => {
     ...
     *  proxy (the surplus-claim fee leg and the net-recipient parameter the
     *  delay needs, in ONE implementation — sequenced, this proxy would be
     *  upgraded twice). Ordered before the BorrowerOperations swap. */
    const collSurplusPoolProxy = await ethers.getContract("CollSurplusPool_Proxy");
    const poolProxyOwner = await collSurplusPoolProxy.getOwner();
    if (
    ...
    if (
        ...
        throw new Error(
            "Perimeter: the CollSurplusPool upgrade must be the first action and the " +
                "BorrowerOperations upgrade the second — the pool's claimCollWithFee selector has " +
                "to exist before the surplus surface can be activated, and there is no in-code " +
                "fallback around the pool call."
        );
    }
    const boSetters = [
