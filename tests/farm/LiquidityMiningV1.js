    ...
    describe("Migration", () => {
        ...
        describe("Grace period start", () => {
            it("should be deployed with state being before grace period", async () => {
                const migrationGracePeriodState =
                    await liquidityMining.migrationGracePeriodState();
                expect(migrationGracePeriodState.toNumber()).to.equal(
                    MigrationGracePeriodStates.None
                );
            });

            it("should only allow grace period to be started by the admin", async () => {
                await expectRevert(
                    liquidityMining.startMigrationGracePeriod({ from: account1 }),
                    "unauthorized"
                );
            });
            ...
            it("should properly start grace period", async () => {
                ...
                const migrationGracePeriodState =
                    await liquidityMining.migrationGracePeriodState();
                expect(migrationGracePeriodState.toNumber()).to.equal(
                    MigrationGracePeriodStates.Started
                );
            ...
            it("should fail when trying to start the grace period if already started", async () => {
                ...
                await expectRevert(
                    liquidityMining.startMigrationGracePeriod(),
            ...
            it("should not let users to deposit", async () => {
                await liquidityMining.startMigrationGracePeriod();
                await expectRevert(
                    liquidityMining.deposit(token1.address, new BN(10), ZERO_ADDRESS, {
                        from: account1,
                    }),
                    "Forbidden: migration already started"
                );
            ...
            it("should not let lending pools to deposit", async () => {
                await liquidityMining.startMigrationGracePeriod();
                await expectRevert(
                    liquidityMining.onTokensDeposited(token1.address, new BN(10), {
                        from: account1,
                    }),
                    "Forbidden: migration already started"
                );
        ...
        describe("Grace period finish", () => {
            it("should only allow admin to finish the grace period", async () => {
                await expectRevert(
                    liquidityMining.finishMigrationGracePeriod({ from: account1 }),
                    "unauthorized"
                );
            });
            ...
            it("should fail if grace period has already finished", async () => {
                ...
                const migrationGracePeriodState =
                    await liquidityMining.migrationGracePeriodState();
                expect(migrationGracePeriodState.toNumber()).to.equal(
                    MigrationGracePeriodStates.Finished
                );
            ...
            it("should not let users to withdraw", async () => {
                ...
                await liquidityMining.finishMigrationGracePeriod();
                await expectRevert(
                    liquidityMining.withdraw(token1.address, new BN(10), account1),
                    "Forbidden: contract deprecated"
                );
                await expectRevert(
                    liquidityMining.emergencyWithdraw(token1.address),
                    "Forbidden: contract deprecated"
                );
            });
            ...
            it("should not let users to claim rewards", async () => {
                ...
                await liquidityMining.finishMigrationGracePeriod();
                await expectRevert(
                    liquidityMining.claimRewardFromAllPools(account1),
                    "Forbidden: contract deprecated"
                );

                await expectRevert(
                    liquidityMining.claimReward(token1.address, account1),
                    "Forbidden: contract deprecated"
                );
            });
            ...
            it("should not let users to update the pools information", async () => {
                ...
                await liquidityMining.finishMigrationGracePeriod();
                await expectRevert(
                    liquidityMining.updatePool(token1.address),
                    "Forbidden: contract deprecated"
                );
                await expectRevert(
