...
const verify_start_conditions = async (
    underlyingToken,
    loanToken,
    lender,
    initial_balance,
...
const lend_btc_before_cashout = async (loanToken, total_deposit_amount, lender) => {
    ...
    expect(await loanToken.checkpointPrice(lender)).to.be.a.bignumber.equal(
        await loanToken.initialPrice()
    );

    const loan_token_initial_balance = total_deposit_amount
        .div(await loanToken.initialPrice())
        .mul(new BN(wei("1", "ether")));
...
const lend_tokens_before_cashout = async (
    loanToken,
    underlyingToken,
...
const verify_lending_result_and_itoken_price_change = async (
    ...
    expect(await loanToken.balanceOf(lender)).to.be.a.bignumber.equal(
        deposit_amount.div(await loanToken.initialPrice()).mul(oneEth)
    );
    const earned_interests_1 = new BN(0); // Shouldn't be earned interests
    const price1 = get_itoken_price(
        deposit_amount,
        earned_interests_1,
        await loanToken.totalSupply()
    );
    expect(await loanToken.tokenPrice()).to.be.a.bignumber.equal(price1);
    expect(await loanToken.checkpointPrice(lender)).to.be.a.bignumber.equal(
        await loanToken.initialPrice()
    );
    expect(await loanToken.totalSupplyInterestRate(deposit_amount)).to.be.a.bignumber.equal(
        new BN(0)
    );
    ...
    const earned_interest_2 = new BN(lender_interest_data["interestUnPaid"])
        ...
        .div(hunEth);
    expect(price_2).to.be.a.bignumber.equal(
        get_itoken_price(deposit_amount, earned_interest_2, await loanToken.totalSupply())
    );
};
...
const lend_to_the_pool = async (loanToken, lender, underlyingToken, collateralToken, sovryn) => {
    ...
    expect(await underlyingToken.balanceOf(lender)).to.be.a.bignumber.equal(
        initial_balance.sub(deposit_amount)
    );
...
const cash_out_from_the_pool = async (loanToken, lender, underlyingToken, lendBTC) => {
    ...
    if (lendBTC) {
        ...
        initial_balance = await lend_tokens_before_cashout(
            loanToken,
            underlyingToken,
            total_deposit_amount,
            lender
...
const cash_out_from_the_pool_uint256_max_should_withdraw_total_balance = async (
    loanToken,
    lender,
    underlyingToken
) => {
...
const set_fee_tokens_held = async (
    sovryn,
    underlyingToken,
    lendingFee,
    tradingFee,
