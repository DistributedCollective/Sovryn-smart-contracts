...
// npx hardhat test ./tests-onchain/fast_btc_pause_freeze.test.js --network rskForkedMainnet
//@note     hh test ./tests-onchain/fast_btc_pause_freeze.test.js --network rskForkedMainnet
...
describe("Freeze fastbtc - Check is frozen - Unfreeze fastbtc", () => {
    let snapshot;
    ...
    it("freezes fastbtc", async () => {
        ...
        await hre.run("pausing:freeze-fastbtc", { signer: "exchequerOwner0" });
    ...
    it("checks fastbtc is frozen", async () => {
        const isFrozen = await (await ethers.getContract("FastBTCBiDi")).frozen();
        LOG(col.yellowBright("    isFrozen?: ") + col.green(isFrozen.toString()));
        assert(isFrozen);
    });

    it("unfreezes fastbtc", async () => {
        const multisig = await ethers.getContract("MultiSigWallet");
        const txCountBefore = await multisig.transactionCount();
        LOG(col.yellowBright("    txCountBefore: ") + col.green(txCountBefore.toString()));
        const submitterAcc = (await hre.getNamedAccounts())["exchequerOwner3"];
        await setBalance(submitterAcc, ONE_RBTC);
        await hre.run("pausing:unfreeze-fastbtc", { signer: "exchequerOwner3" });
        const txCountAfter = await multisig.transactionCount();
        LOG(col.yellowBright("    txCountAfter: ") + col.green(txCountAfter.toString()));
    ...
    it("checks fastbtc is unfreezed", async () => {
        const isFrozen = await (await ethers.getContract("FastBTCBiDi")).frozen();
        LOG(col.yellowBright("    isFrozen?: ") + col.green(isFrozen.toString()));
        assert(!isFrozen);
    });
...
describe("Pause fastbtc - Check is paused - Unpause fastbtc", () => {
    let snapshot;
    ...
    it("pauses fastbtc", async () => {
        const multisig = await ethers.getContract("MultiSigWallet");
        const txCountBefore = await multisig.transactionCount();
        LOG(col.yellowBright("    txCountBefore: ") + col.green(txCountBefore.toString()));
        const submitterAcc = (await hre.getNamedAccounts())["exchequerOwner0"];
        await setBalance(submitterAcc, ONE_RBTC);
        await hre.run("pausing:pause-fastbtc", { signer: "exchequerOwner0" });
        const txCountAfter = await multisig.transactionCount();
        LOG(col.yellowBright("    txCountAfter: ") + col.green(txCountAfter.toString()));
    ...
    it("checks fastbtc is paused", async () => {
        const isPaused = await (await ethers.getContract("FastBTCBiDi")).paused();
        LOG(col.yellowBright("    isPaused?: ") + col.green(isPaused.toString()));
        assert(isPaused);
    });

    it("unpauses fastbtc", async () => {
        const multisig = await ethers.getContract("MultiSigWallet");
        const txCountBefore = await multisig.transactionCount();
        LOG(col.yellowBright("    txCountBefore: ") + col.green(txCountBefore.toString()));
        const submitterAcc = (await hre.getNamedAccounts())["exchequerOwner3"];
        await setBalance(submitterAcc, ONE_RBTC);
        await hre.run("pausing:unpause-fastbtc", { signer: "exchequerOwner3" });
        const txCountAfter = await multisig.transactionCount();
        LOG(col.yellowBright("    txCountAfter: ") + col.green(txCountAfter.toString()));
    ...
    it("checks fastbtc is unpaused", async () => {
        const isPaused = await (await ethers.getContract("FastBTCBiDi")).paused();
        LOG(col.yellowBright("    isPaused?: ") + col.green(isPaused.toString()));
        assert(!isPaused);
    });
