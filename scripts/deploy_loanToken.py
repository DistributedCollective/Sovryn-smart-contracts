...
def deployLoanTokens(acct, sovryn, tokens):
    ...
    print('\n DEPLOYING ISUSD')
    (contractSUSD, loanTokenSettingsSUSD) = deployLoanToken(acct, sovryn, tokens.susd.address, "SUSD", "SUSD", tokens.rbtc.address, tokens.weth.address)
    ...
    print('\n DEPLOYING IRBTC')
    (contractRBTC, loanTokenSettingsRBTC) = deployLoanToken(acct, sovryn, tokens.rbtc.address, "RBTC", "RBTC", tokens.susd.address, tokens.weth.address)
    print("initializing the lending pool with some tokens, so we do not run out of funds")
