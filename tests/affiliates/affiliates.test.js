...
contract("Affiliates", (accounts) => {
    ...
    it("User Margin Trade with Affiliate runs correctly", async () => {
        ...
        expect(
            await sovryn.getUserNotFirstTradeFlag(trader),
            "userNotFirstTradeFlag has not been set to true"
        ).to.be.true;
        expect(
            await sovryn.getAffiliatesUserReferrer(trader),
            "Incorrect User Affiliate Referrer set"
        ).to.be.equal(referrer);
        ...
        ).to.be.equal(submittedSovBonusAmount);
        ...
        expect(
            ...
            "SOV Bonus amount that stored in the affiliateRewardsHeld is incorrect"
        ).to.be.equal(sovBonusAmountShouldBePaid.toString());
        expect(
            isHeld,
            "Token should not be sent since the minimum referrals to payout has not been fullfilled"
        ).to.eql(true);

        lockedSOVBalance = await lockedSOV.getLockedBalance(referrer);
        expect(lockedSOVBalance.toString(), "Locked sov balance should be 0").to.eql(
            new BN(0).toString()
        );
        ...
        expect(
            sovBonusAmountShouldBePaid.toString(),
            "Incorrect sov bonus amount calculation"
        ).to.be.equal(submittedSovBonusAmount.toString());
        expect(submittedToken).to.eql(doc.address);
        expect(submittedReferrer).to.eql(referrer);
        expect(submittedTrader).to.eql(trader);

        expect(
            new BN(parseInt(previousAffiliateRewardsHeld))
                .add(new BN(parseInt(sovBonusAmountShouldBePaid)))
                .toString(),
            "Incorrect sov bonus amount calculation"
        ).to.be.equal(paidSovBonusAmount.toString());
        expect(
            affiliateRewardsHeld.toString(),
            "Affiliates rewards should be 0 after rewards is sent"
        ).to.eql(new BN(0).toString());
        expect(
            isHeld,
            "Token should be sent since the minimum referrals to payout has not been fullfilled"
        ).to.eql(false);

        lockedSOVBalance = await lockedSOV.getLockedBalance(referrer);
        expect(lockedSOVBalance.toString(), "Locked sov balance should be 0").to.eql(
            paidSovBonusAmount.toString()
        );
    });
    ...
    it("User Margin Trade with Affiliate runs correctly when minimum referrals set to 1", async () => {
        ...
        lockedSOVBalance = await lockedSOV.getLockedBalance(referrer);
        expect(lockedSOVBalance.toString(), "Locked sov balance is not matched").to.eql(
            sovBonusAmountShouldBePaid.toString()
        );
    });
    ...
    it("Only the first trade users can be assigned Affiliates Referrer", async () => {
        ...
        expect(
            await sovryn.getAffiliatesUserReferrer(trader),
            "Referrer cannot be set for non first trade users"
    ...
    it("Affiliates Referrer cannot be changed once set", async () => {
        ...
        expect(
            await sovryn.getAffiliatesUserReferrer(trader),
            "Affiliates Referrer is set once and cannot be changed"
        ).to.be.equal(referrer);
    });
    ...
    it("First users Margin trading without affiliate referrer sets userNotFirstTradingFlag = true", async () => {
        ...
        expect(
            await sovryn.getUserNotFirstTradeFlag(trader),
            "sovryn.getUserNotFirstTradeFlag(trader) should be true"
        ).to.be.true;
    });
    ...
    it("payTradingFeeToAffiliatesReferrer() Should revert if not called by protocol", async () => {
        await expectRevert(
            sovryn.payTradingFeeToAffiliatesReferrer(
                referrer,
                trader,
                SUSD.address,
                wei("1", "gwei")
    ...
    it("setAffiliatesReferrer() should revert if not called by loanPool", async () => {
        await expectRevert(
            sovryn.setAffiliatesReferrer(accounts[0], referrer),
            "Affiliates: not authorized"
        );
    });
    ...
    it("setUserNotFirstTradeFlag() should revert if not called by loanPool", async () => {
        await expectRevert(
            sovryn.setUserNotFirstTradeFlag(referrer),
            "Affiliates: not authorized"
        );
    });
    ...
    it("Withdraw all token will revert if receiver is zero address", async () => {
        await expectRevert(
            sovryn.withdrawAllAffiliatesReferrerTokenFees(constants.ZERO_ADDRESS, {
                from: referrer,
            }),
    ...
    it("Affiliates referrer withdraw fees should revert because of the minimum request is not fullfilled", async () => {
        ...
        const referrerFee = await sovryn.affiliatesReferrerBalances(referrer, doc.address);
        await expectRevert(
            sovryn.withdrawAffiliatesReferrerTokenFees(
                doc.address,
        ...
        await expectRevert(
            sovryn.withdrawAffiliatesReferrerTokenFees(doc.address, referrer, 0, {
                from: referrer,
            }),
        ...
        await expectRevert(
            sovryn.withdrawAffiliatesReferrerTokenFees(doc.address, referrer, referrerFee, {
                from: referrer,
            }),
            "Your referrals has not reached the minimum request"
        );
    ...
    it("Affiliates Referrer withdraw fees in two tokens works correctly with min referrals to payout is 0", async () => {
        ...
        const loanToken2 = await LoanToken.new(
            owner,
            loanTokenLogic.address,
            sovryn.address,
            WRBTC.address
		...
		*/
        ...
        const kinkLevel = wei("90", "ether");
        const maxScaleRate = wei("100", "ether");
        await loanToken2V2.setDemandCurve(
            baseRate,
            rateMultiplier,
            baseRate,
            rateMultiplier,
        ...
        await expectRevert(
            sovryn.withdrawAffiliatesReferrerTokenFees(eur.address, referrer, 0, {
                from: referrer,
            }),
            "Affiliates: cannot withdraw zero amount"
        );
        ...
        // partial withdraw
        let tx;
        tx = await sovryn.withdrawAffiliatesReferrerTokenFees(
            doc.address,
            referrer,
            referrerFee.divn(2),
            { from: referrer }
        ...
        expect(
            newReferrerFee,
            "Incorrect partial balance after half DoC withdraw"
        ).to.be.bignumber.equal(referrerFee.divn(2));
        ...
        await sovryn.withdrawAffiliatesReferrerTokenFees(
            eur.address,
            referrer,
            referrerFee2.divn(4),
            { from: referrer }
        ...
        expect(
            newReferrerFee2,
            "Incorrect partial balance after a quarter EUR withdraw"
        ).to.be.bignumber.equal(referrerFee2.muln(3).divn(4));

        let refBalances = await sovryn.getAffiliatesReferrerBalances(referrer);

        // complete withdraw
        await sovryn.withdrawAffiliatesReferrerTokenFees(doc.address, referrer, newReferrerFee, {
            from: referrer,
        });
        expect(
            await doc.balanceOf(referrer),
            "Incorrect DoC withdraw amount"
        ).to.be.bignumber.equal(referrerFee);
        expect(
        ...
        expect(refBalances["referrerTokensBalances"][0]).to.be.bignumber.equal(
            new BN(Math.pow(10, 15)).muln(18)
        );
        expect(refBalances["referrerTokensBalances"]).to.have.length(1);

        await sovryn.withdrawAffiliatesReferrerTokenFees(eur.address, referrer, newReferrerFee2, {
            from: referrer,
        });
        expect(
            await eur.balanceOf(referrer),
            "Incorrect EUR withdraw amount"
        ).to.be.bignumber.equal(referrerFee2);
        expect(
        ...
        refBalances = await sovryn.getAffiliatesReferrerBalances(referrer);
        expect(
            refBalances["referrerTokensList"][0],
            "After withdrawal the token should be deleted from the referrers list"
        ).to.be.undefined;
        expect(
            refBalances["referrerTokensBalances"][0],
            "After withdrawal the token balances should be deleted from the referrers list"
        ).to.be.undefined;
    });
    ...
    it("Affiliates Referrer withdraw all fees for two tokens works correctly with min referrals to payout is 0", async () => {
        ...
        expect(
            newReferrerFee2,
            "Incorrect all balance after all EUR withdraw"
        ).to.be.bignumber.equal(new BN(0));

        await expectEvent.inTransaction(
            tx.receipt.rawLogs[1].transactionHash,
            Affiliates,
            "WithdrawAffiliatesReferrerTokenFees",
            {
                referrer: referrer,
