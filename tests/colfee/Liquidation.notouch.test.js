 ...
 *
 * This test asserts: when an unhealthy position is liquidated, NO ColFee
 * event is emitted, regardless of controller state.
 *
 * Run:
 *   npx hardhat test tests/colfee/Liquidation.notouch.test.js
 */
...
contract("ColFee — Liquidation no-touch coverage (Phase 3 / Task 3.4)", (accounts) => {
    ...
    async function deploymentAndInitFixture() {
        ...
        // ColFee controller FULLY ACTIVE — proves the no-touch property
    ...
    describe("liquidate (allowDonationOnFailure=true gate)", () => {
        // POSITIVE CONTROL for the no-touch claim below. Without it, "no ColFee
        // event on liquidation" is indistinguishable from "the ColFee system is
        ...
        it("ColFee does NOT fire when an unhealthy position is liquidated — no ExitFeeApplied, no ExitFeeSkipped", async () => {
