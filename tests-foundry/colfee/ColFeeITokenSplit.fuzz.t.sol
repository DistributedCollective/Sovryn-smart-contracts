...
/// protocol-side split fuzzed in `ColFeeSplit.fuzz.t.sol`.
///
/// The two hooks are hand-written twins: the protocol tree charges through a
/// delegatecall into `ColFeeBorrowerExitOps`, the iToken tree charges inline
...
/// `contracts/mockup/colfee/MockArbitraryQuoteExitFeeController.sol`.
...
/// (`ColFeeLib.safeControllerLookup(sovrynContractAddress)`), so the harness
...
contract ColFeeITokenSplitFuzzTest {
    ...
    function testFuzz_inlineInvalidQuotePaysFullGross(
        ...
        vm.assume(!ColFeeLib.quoteIsValid(q, gross));
