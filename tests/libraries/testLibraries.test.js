 ...
 * No bottlenecks found. Test flow seems to be flawless.
 *
 * Minor optimizations:
...
contract("TestLibraries", (accounts) => {
    ...
    describe("test RSKAddrValidator", async () => {
        it("checkPKNotZero(PUB_KEY_FROM_ZERO_ADDRESS) == false", async () => {
            ...
            expect(await testLibraries.RSKAddrValidator_checkPKNotZero(PUB_KEY_FROM_ZERO)).to.be
                .false;
        });

        it("checkPKNotZero(ZERO_ADDRESS) == true", async () => {
            expect(await testLibraries.RSKAddrValidator_checkPKNotZero(constants.ZERO_ADDRESS)).to
                .be.false;
        });
        ...
        it("safeEquals(PUB_KEY_FROM_ZERO, PUB_KEY_FROM_ZERO) == false", async () => {
            expect(
                await testLibraries.RSKAddrValidator_safeEquals(
                    PUB_KEY_FROM_ZERO,
                    PUB_KEY_FROM_ZERO
                )
            ).to.be.false;
        ...
        it("safeEquals(account1, ZERO_ADDRESS) == false", async () => {
            expect(
                await testLibraries.RSKAddrValidator_safeEquals(constants.ZERO_ADDRESS, account1)
            ).to.be.false;
            expect(
                await testLibraries.RSKAddrValidator_safeEquals(account1, constants.ZERO_ADDRESS)
            ).to.be.false;
        });

        it("safeEquals(account1, account2 || account2, account1) == false", async () => {
            expect(await testLibraries.RSKAddrValidator_safeEquals(account2, account1)).to.be
                .false;
            expect(await testLibraries.RSKAddrValidator_safeEquals(account1, account2)).to.be
                .false;
        });
    });
});
